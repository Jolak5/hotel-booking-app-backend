...
RSpec.describe 'Hotels', type: :request do
    ...
    describe 'GET /hotels/:id' do
        it 'returns http success' do
          get "/hotels/#{hotel.id}", headers: headers
          expect(response.status).to eq(200)
        end
    ...
    describe 'POST /create' do
        it 'creates a hotel' do
            post "/hotels", params: { hotel: { name: 'Hotel California', description: 'A lovely place', duration: 5, price: 200.0 } }, headers: headers
            expect(response.status).to eq(201)
        end

        it 'fails to create a hotel with missing params' do
            post "/hotels", params: { hotel: { name: 'Hotel California', description: 'A lovely place', duration: 5 } }, headers: headers
    ...
    describe 'DELETE /delete' do
        it 'deletes a hotel' do
            delete "/hotels/#{hotel.id}", headers: headers
