...
RSpec.describe 'Reservations', type: :request do
    ...
    let(:reservation) do
      Reservation.create!(user: user, hotel: hotel, reservation_date: Date.today, duration: 5)
    end
  ...
  describe 'POST /create' do
    it 'creates a reservation' do
      post "/reservations", params: { reservation: { user_id: user.id, hotel_id: hotel.id, reservation_date: Date.tomorrow, duration: 2 } }, headers: headers
      expect(response.status).to eq(201)
    end

    it 'fails to create a reservation with missing params' do
      post "/reservations", params: { reservation: { user_id: user.id, duration: 2 } }, headers: headers
